...
| `mailbourne-inspect` | **The judge.** Turns probe evidence into a checklist report (WHAT/WHY/DO/VERIFY/LEARN per item). Returns data; renders nothing. | Why setup fails and how each fix is verified. |
...
1. **Engines return data, renderers render.** `mailbourne-inspect` and
